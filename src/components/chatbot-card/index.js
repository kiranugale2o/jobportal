...
import { suggestQuestions } from "@/utils";
...
export default function ChatbotCard() {
  ...
  return (
    <div className="w-full p-0  bg-white lg:p-20   mx-0">
      ...
      <div className="fixed bottom-10  lg:bottom-1 lg:right-8">
        <DropdownMenu className="overscroll-contain">
          <DropdownMenuTrigger asChild>
            <Button variant="outline">Suggest Qestions</Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-[300px] lg:w-full">
            <DropdownMenuLabel>Suggest Qestions</DropdownMenuLabel>
            {suggestQuestions && suggestQuestions.length > 0
              ? suggestQuestions.map((d, i) => {
                  return (
                    <>
                      <DropdownMenuItem
                        onClick={(e) => {
                          setUserInput(d.question);
                        }}
                      >
                        {i + 1}
                        {")"}
